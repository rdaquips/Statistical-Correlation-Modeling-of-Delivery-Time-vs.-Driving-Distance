...
--------------------------------------------------------------------------------------------------------------------------------------------------------------
    ...
    SELECT
        ...
        CASE 
            ...
            ELSE 'Untagged'
        END AS delivery_performance,
        distance_rider_from_store,
        distance_to_customer,
        rider_arrived_at_pickup_time,
        order_ready_for_pickup_time,
        order_pickup_done_time,
        order_delivered_time
    FROM [SLvsDistance])

    -- For completed orders findings--
    SELECT
        order_id,
    ...
    ORDER BY mins_total_service DESC;

    -- For cancelled orders findings--
    SELECT
        order_id,
        COUNT(*) AS stage_0,
    ...
    FROM IS_SQL_Cancelled_Orders
    WHERE order_status_label = 'Cancelled'
    GROUP BY order_id
